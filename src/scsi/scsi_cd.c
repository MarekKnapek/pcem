...
static struct {
        ...
        {1, 240, 1446}, {2, 160, 1000}, {3, 150, 900}, {4, 112, 675}, {6, 112, 675}, {8, 112, 675},
        {12, 75, 400},  {16, 58, 350},  {20, 50, 300}, {24, 45, 270}, {32, 45, 270}, {36, 45, 270},
        {40, 50, 300},  {44, 50, 300},  {48, 50, 300}, {52, 45, 270}, {56, 45, 270}, {72, 45, 270},
};

int cd_speed;

int cd_get_speed(int i) { return cd_speeds[i].speed; }
...
int cd_get_model_interfaces(int i) { return cd_models[i].interfaces; }
...
static void scsi_cd_illegal(scsi_cd_data_t *data) { atapi_cmd_error(data, KEY_ILLEGAL_REQ, ASC_INVALID_LUN, 0); }
...
uint32_t atapi_get_cd_channel(int channel) {
        return (page_flags[GPMODE_CDROM_AUDIO_PAGE] & PAGE_CHANGED) ? mode_pages_in[GPMODE_CDROM_AUDIO_PAGE][channel ? 8 : 6]
                                                                    : (channel + 1);
}

uint32_t atapi_get_cd_volume(int channel) {
        return (page_flags[GPMODE_CDROM_AUDIO_PAGE] & PAGE_CHANGED) ? mode_pages_in[GPMODE_CDROM_AUDIO_PAGE][channel ? 9 : 7]
                                                                    : 0xFF;
}
static int scsi_cd_command(uint8_t *cdb, void *p) {
        ...
        case SCSI_REQUEST_SENSE: /* Used by ROS 4+ */
                ...
                if ((data->sense_key > 0) || (data->cd_status < CD_STATUS_PLAYING)) {
                        ...
                        data->data_in[2] = SENSE_ILLEGAL_REQUEST;
                        data->data_in[12] = ASC_AUDIO_PLAY_OPERATION;
                        data->data_in[13] = (data->cd_status == CD_STATUS_PLAYING) ? ASCQ_AUDIO_PLAY_OPERATION_IN_PROGRESS
                                                                                   : ASCQ_AUDIO_PLAY_OPERATION_PAUSED;
                }
        ...
        case GPCMD_READ_CD:
                //                pclog("Read CD : start LBA %02X%02X%02X%02X Length %02X%02X%02X Flags
        ...
        case GPCMD_READ_12:
                ...
                if (data->cmd_pos == CMD_POS_IDLE) {
                        ...
                        if (cdb[0] == GPCMD_READ_6) {
                                ...
                                data->cdlen = ((uint32_t)cdb[6] << 24) | ((uint32_t)cdb[7] << 16) | ((uint32_t)cdb[8] << 8) |
                                              (uint32_t)cdb[9];
                                data->cdpos = ((uint32_t)cdb[2] << 24) | ((uint32_t)cdb[3] << 16) | ((uint32_t)cdb[4] << 8) |
                                              (uint32_t)cdb[5];
                        }
        ...
        case GPCMD_INQUIRY:
                ...
                if (cdb[1] & 1) {
                        ...
                        case 0x83:
                                ...
                                idx += 8;
                                ide_padstr8(data->data_in + idx, 40,
