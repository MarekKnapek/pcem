...
startrep:
        ...
        case 0xA6: /*REP CMPSB*/
                ...
                //                FETCHCLEAR(); } else firstrepcycle=1;
                break;
        case 0xA7: /*REP CMPSW*/
                if (fv)
                        cpu_state.flags |= Z_FLAG;
                ...
                if (IRQTEST && c > 0 && (fv == ((cpu_state.flags & Z_FLAG) ? 1 : 0)))
                        cpu_state.pc = ipc;
                //                if ((c>0) && (fv==((flags&Z_FLAG)?1:0))) { pc=ipc; firstrepcycle=0; if (ssegs) ssegs++;
                //                FETCHCLEAR(); } else firstrepcycle=1; if (firstrepcycle) printf("REP CMPSW  %06X:%04X %06X:%04X
...
void execx86(int cycs) {
        ...
        opcodestart:
                ...
                cpu_state.pc--;
                //                output=1;
                //                if (output) printf("%04X:%04X : %04X %04X %04X %04X %04X %04X %04X %04X %04X %04X %04X %04X %02X
                ...
                if (output) {
                        ...
                        //                        {
                        if (!skipnextprint)
                                printf("%04X:%04X : %04X %04X %04X %04X %04X %04X %04X %04X %04X %04X %04X %04X %02X %04X  %i %p "
                                       "%02X\n",
                                       cs, cpu_state.pc, AX, BX, CX, DX, CS, DS, ES, SS, DI, SI, BP, SP, opcode, cpu_state.flags,
                                       ins, ram, ram[0x1a925]);
                        skipnextprint = 0;
                        //                                ins++;
                        //                        }
                ...
                case 0x76: /*JBE*/
                        offset = (int8_t)FETCH();
                        if (cpu_state.flags & (C_FLAG | Z_FLAG)) {
                                cpu_state.pc += offset;
                                cycles -= 12;
                                FETCHCLEAR();
                        }
                ...
                case 0x7E: /*JLE*/
                        ...
                        temp = (cpu_state.flags & N_FLAG) ? 1 : 0;
                        temp2 = (cpu_state.flags & V_FLAG) ? 1 : 0;
                        if ((cpu_state.flags & Z_FLAG) || (temp != temp2)) {
                                cpu_state.pc += offset;
                                cycles -= 12;
                                FETCHCLEAR();
                        }
                ...
                case 0x8E: /*MOV sreg,w*/
                        ...
                        case 0x10: /*SS*/
                                ...
                                //				printf("SS loaded with %04X %04X:%04X %04X %04X
                ...
                case 0xD1:
                        ...
                        case 0x00: /*ROL w,1*/
                                ...
                                seteaw(tempw);
                                //                                setznp16(tempw);
                                if ((cpu_state.flags & C_FLAG) ^ (tempw >> 15))
                                        cpu_state.flags |= V_FLAG;
                                else
                                        cpu_state.flags &= ~V_FLAG;
                                cycles -= ((cpu_mod == 3) ? 2 : 23);
                ...
                case 0xF6:
                        ...
                        case 0x30: /*DIV AL,b*/
                                ...
                                if (temp) {
                                        ...
                                        //                                                cs=loadcs(CS);
                                        //                                                cs=CS<<4;
                                        //                                        printf("Div by zero %04X:%04X %02X
                                        //                                        %02X\n",cs>>4,pc,0xf6,0x30); dumpregs();
                                        //                                        exit(-1);
                                }
                                cycles -= 80;
                                break;
                ...
                case 0xFF:
                        ...
                        case 0x30: /*PUSH w*/
                                tempw = geteaw();
                                //                                if (output) printf("PUSH %04X %i %02X %04X %04X %02X
                ...
                default:
                        ...
                        /*                        printf("Bad opcode %02X at %04X:%04X from %04X:%04X
                           %08X\n",opcode,cs>>4,pc,old8>>16,old8&0xFFFF,old82); dumpregs(); exit(-1);*/
                }
                cpu_state.pc &= 0xFFFF;
