...
static void prefetch_run(int instr_cycles, int bytes, int modrm, int reads, int reads_l, int writes, int writes_l, int ea32) {
        int mem_cycles =
                reads * cpu_cycles_read + reads_l * cpu_cycles_read_l + writes * cpu_cycles_write + writes_l * cpu_cycles_write_l;
...
static void prefetch_flush() { prefetch_bytes = 0; }
...
static inline void exec_interpreter(void) {
        ...
        while (!cpu_block_end) {
                ...
                if (!cpu_state.abrt) {
                        ...
                        //                        if (output == 3)
                        //                                pclog("int %04X(%06X):%04X : %08X %08X %08X %08X %04X %04X %04X(%08X)
                        //                                %04X %04X %04X(%08X) %08X %08X %08X SP=%04X:%08X %02X %04X %i %08X  %08X
                        //                                %i %i %02X %02X %02X   %02X %02X %f  %02X%02X
                        //                                %02X%02X\n",CS,cs,pc,EAX,EBX,ECX,EDX,CS,DS,ES,es,FS,GS,SS,ss,EDI,ESI,EBP,SS,ESP,opcode,flags,ins,0,
                        //                                ldt.base, CPL, stack32, pic.pend, pic.mask, pic.mask2, pic2.pend,
                        //                                pic2.mask, pit.c[0], ram[0x8f13f], ram[0x8f13e], ram[0x8f141],
                        //                                ram[0x8f140]);
...
static void exec_recompiler(void) {
        ...
        if (!cpu_state.abrt) {
                ...
                valid_block = (block->pc == cs + cpu_state.pc) && (block->_cs == cs) && (block->phys == phys_addr) &&
                        ...
                        if ((page->code_present_mask & mask) || (page->byte_code_present_mask[byte_offset] & byte_mask)) {
                                ...
                                if (new_block) {
                                        valid_block = (new_block->pc == cs + cpu_state.pc) && (new_block->_cs == cs) &&
                                                      ...
                                                      ((new_block->status & cpu_cur_status & CPU_STATUS_MASK) ==
                ...
                if (valid_block && (block->flags & CODEBLOCK_WAS_RECOMPILED) && (block->flags & CODEBLOCK_STATIC_TOP) &&
        ...
        if (valid_block && (block->flags & CODEBLOCK_WAS_RECOMPILED)) {
                ...
                //                %08x  %016llx %08x\n", CS, pc, AX, BX, CX, DX, SI, DI, ESP, BP, get_phys(cs+pc), block->phys,
                //                block->page_mask, block->endpc);
                ...
                //                if (output) pclog("Recompile block at %04x:%04x  %04x %04x %04x %04x  %04x %04x  ESP=%04x %04x
                //                %02x%02x:%02x%02x %02x%02x:%02x%02x %02x%02x:%02x%02x\n", CS, pc, AX, BX, CX, DX, SI, DI, ESP,
                //                BP, ram[0x116330+0x6df4+0xa+3], ram[0x116330+0x6df4+0xa+2], ram[0x116330+0x6df4+0xa+1],
                //                ram[0x116330+0x6df4+0xa+0], ram[0x11d136+3],ram[0x11d136+2],ram[0x11d136+1],ram[0x11d136+0],
                //                ram[(0x119abe)+0x3],ram[(0x119abe)+0x2],ram[(0x119abe)+0x1],ram[(0x119abe)+0x0]);
                while (!cpu_block_end) {
                        cpu_state.oldpc = cpu_state.pc;
                        cpu_state.op32 = use32;
                        ...
                        if (!cpu_state.abrt) {
                                ...
                                //                                if (output == 3)
                                //                                        pclog("%04X(%06X):%04X : %08X %08X %08X %08X %04X %04X
                                //                                        %04X(%08X) %04X %04X %04X(%08X) %08X %08X %08X
                                //                                        SP=%04X:%08X %02X %04X %i %08X  %08X %i %i %02X %02X
                                //                                        %02X   %02X %02X  %08x
                                //                                        %08x\n",CS,cs,pc,EAX,EBX,ECX,EDX,CS,DS,ES,es,FS,GS,SS,ss,EDI,ESI,EBP,SS,ESP,opcode,flags,ins,0,
                                //                                        ldt.base, CPL, stack32, pic.pend, pic.mask, pic.mask2,
                                //                                        pic2.pend, pic2.mask, cs+pc, pccache);

                                cpu_state.pc++;

                                codegen_generate_call(opcode, x86_opcodes[(opcode | cpu_state.op32) & 0x3ff], fetchdat,
