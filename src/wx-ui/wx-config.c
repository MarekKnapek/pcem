...
static void recalc_hdd_list(void *hdlg, int model, int use_selected_hdd, int force_ide) {
        ...
        while (1) {
                ...
                if ((((hdd_controller_get_flags(c) & DEVICE_AT) && !(models[model]->flags & MODEL_AT)) ||
                     (hdd_controller_get_flags(c) & DEVICE_MCA) != (models[model]->flags & MODEL_MCA)) &&
                    c) {
                        c++;
                        continue;
                }
        ...
        if (hdd_controller_is_scsi(hdd_names[hdd_type])) {
                ...
                for (c = 0; c < 4; c++) {
                        ...
                        page = wx_getdlgitem(hdlg, WX_ID(s));
                        wx_sendmessage(page, WX_REPARENT, (LONG_PARAM)h, 0);
                        sprintf(s, "Drive %i %s %s (%c:)", c, (c & 2) ? "Secondary" : "Primary", (c & 1) ? "Slave" : "Master",
                                'C' + c);
                        wx_sendmessage(h, WX_CHB_SETPAGETEXT, c, (LONG_PARAM)s);
                }
                for (c = 6; c >= 4; c--) {
...
int config_dlgproc(void *hdlg, int message, INT_PARAM wParam, LONG_PARAM lParam) {
        ...
        case WX_INITDIALOG: {
                ...
                while (models[romstomodel[romset]]->cpu[c].cpus != NULL && c < 4) {
                        wx_sendmessage(h, WX_CB_ADDSTRING, 0, (LONG_PARAM)models[romstomodel[romset]]->cpu[c].name);
                        c++;
                }
                wx_enablewindow(h, TRUE);
                ...
                wx_sendmessage(h, WX_BM_SETCHECK,
                               ((cpu_flags & CPU_SUPPORTS_DYNAREC) && cpu_use_dynarec) || (cpu_flags & CPU_REQUIRES_DYNAREC), 0);
                ...
                wx_sendmessage(h, WX_UDM_SETRANGE, 0,
                               (models[romstomodel[romset]]->min_ram << 16) | models[romstomodel[romset]]->max_ram);
        ...
        case WX_COMMAND: {
                if (wParam == WX_ID("IDC_COMBO1")) {
                        ...
                        while (models[temp_model]->cpu[c].cpus != NULL && c < 4) {
                                wx_sendmessage(h, WX_CB_ADDSTRING, 0, (LONG_PARAM)models[temp_model]->cpu[c].name);
                                c++;
                        }
                        if (temp_cpu_m >= c)
                                temp_cpu_m = c - 1;
                        ...
                        h = wx_getdlgitem(hdlg, WX_ID("IDC_COMBOFPU"));
                        temp_fpu = fpu_get_type_from_index(temp_model, temp_cpu_m, temp_cpu,
                                                           wx_sendmessage(h, WX_CB_GETCURSEL, 0, 0));
                        ...
                        if (!(cpu_flags & CPU_SUPPORTS_DYNAREC) || (cpu_flags & CPU_REQUIRES_DYNAREC))
                                ...
                                wx_enablewindow(h, TRUE);
                        wx_sendmessage(h, WX_BM_SETCHECK,
                                       ((cpu_flags & CPU_SUPPORTS_DYNAREC) && temp_dynarec) || (cpu_flags & CPU_REQUIRES_DYNAREC),
                                       0);
                        ...
                        wx_sendmessage(h, WX_UDM_SETRANGE, 0, (models[temp_model]->min_ram << 16) | models[temp_model]->max_ram);
                        mem &= ~(models[temp_model]->ram_granularity - 1);
                        if (mem < models[temp_model]->min_ram)
                                mem = models[temp_model]->min_ram;
                        else if (mem > models[temp_model]->max_ram)
...
static struct {
        int cylinders;
        int heads;
} hd_types[] = {{306, 4},   {615, 4},   {615, 6},  {940, 8},  {940, 6},  {615, 4},  {462, 8},  {733, 5}, {900, 15}, {820, 3},
...
static void vhd_progress_callback(uint32_t current_sector, uint32_t total_sectors) { create_drive_pos = (int)current_sector; }
...
static int hdnew_dlgproc(void *hdlg, int message, INT_PARAM wParam, LONG_PARAM lParam) {
        ...
        case WX_INITDIALOG:
                ...
                for (c = 1; c <= 46; c++) {
                        sprintf(s, "Type %02i : cylinders=%i, heads=%i, size=%iMB", c, hd_types[c - 1].cylinders,
                                hd_types[c - 1].heads,
                                (hd_types[c - 1].cylinders * hd_types[c - 1].heads * 17 * 512) / (1024 * 1024));
                        wx_sendmessage(h, WX_CB_ADDSTRING, 0, (LONG_PARAM)s);
                }
                wx_sendmessage(h, WX_CB_SETCURSEL, 0, 0);
        ...
        case WX_COMMAND:
                if (wParam == wxID_OK) {
                        ...
                        if (hd_format == 0) /* Raw .img */
                        {
                                ...
                                wx_progressdialog(hdlg, "PCem", "Creating drive, please wait...", create_drive_raw, f,
                                                  hd_new_cyl * hd_new_hpc * hd_new_spt, &create_drive_pos);
                        } else if (hd_format == 1) /* Fixed VHD */
                        {
                                if (!wx_progressdialog(hdlg, "PCem", "Creating drive, please wait...", create_drive_vhd_fixed,
                                                       NULL, hd_new_cyl * hd_new_hpc * hd_new_spt, &create_drive_pos)) {
                        ...
                        {
                                if (!getfilewithcaption(hdlg, "VHD file (*.vhd)|*.vhd|All files (*.*)|*.*", "",
                        ...
                        if (hd_format != 3) {
                                wx_messagebox(hdlg, "Drive created, remember to partition and format the new drive.", "PCem",
                                              WX_MB_OK);
                        }
                        ...
                        hd_format = wx_sendmessage(h, WX_CB_GETCURSEL, 0, 0);
                        if (!getsfile(hdlg,
                                      hd_format == 0 ? "Raw Hard disc image (*.img)|*.img|All files (*.*)|*.*"
                                                     : "VHD file (*.vhd)|*.vhd|All files (*.*)|*.*",
                                      "", NULL, hd_format == 0 ? "img" : "vhd")) {
                                h = wx_getdlgitem(hdlg, WX_ID("IDC_EDITC"));
                        ...
                        h = wx_getdlgitem(hdlg, WX_ID("IDC_EDIT4"));
                        sprintf(s, "%i",
                                (int)(((((uint64_t)hd[0].tracks * (uint64_t)hd[0].hpc) * (uint64_t)hd[0].spt) * 512) / 1024) /
                                        1024);
                        wx_sendmessage(h, WX_WM_SETTEXT, 0, (LONG_PARAM)s);

                        hd_type = 0;
                        for (c = 1; c <= 46; c++) {
                                pclog("Compare %i,%i %i,%i %i,%i\n", hd[0].spt, 17, hd[0].hpc, hd_types[c - 1].heads,
                                      hd[0].tracks, hd_types[c - 1].cylinders);
                                if (hd[0].spt == 17 && hd[0].hpc == hd_types[c - 1].heads &&
                                    hd[0].tracks == hd_types[c - 1].cylinders) {
                                        hd_type = c;
                                        break;
                                }
                        ...
                        if (hd_type) {
                                ...
                                h = wx_getdlgitem(hdlg, WX_ID("IDC_EDIT4"));
                                sprintf(s, "%i",
                                        (int)(((uint64_t)hd_types[hd_type - 1].cylinders * hd_types[hd_type - 1].heads * 17 *
                                               512) /
                                              1024) /
                                                1024);
                                wx_sendmessage(h, WX_WM_SETTEXT, 0, (LONG_PARAM)s);
...
static int hdsize_dlgproc(void *hdlg, int message, INT_PARAM wParam, LONG_PARAM lParam) {
        ...
        case WX_INITDIALOG:
                ...
                sprintf(s, "%imb",
                        (int)((((uint64_t)hd_new_spt * (uint64_t)hd_new_hpc * (uint64_t)hd_new_cyl) * 512) / 1024) / 1024);
                wx_sendmessage(h, WX_WM_SETTEXT, 0, (LONG_PARAM)s);
...
int hdconf_init(void *hdlg) {
        ...
        for (c = 0; c < 7; c++) {
                ...
                h = wx_getdlgitem(hdlg, WX_ID(s));
                sprintf(s, "%imb",
                        (int)(((((uint64_t)hdc[c].tracks * (uint64_t)hdc[c].hpc) * (uint64_t)hdc[c].spt) * 512) / 1024) / 1024);
                wx_sendmessage(h, WX_WM_SETTEXT, 0, (LONG_PARAM)s);
        }
...
static int hd_new(void *hdlg, int drive) {
        if (wx_dialogbox(hdlg, "HdNewDlg", hdnew_dlgproc) == 1) {
                ...
                h = wx_getdlgitem(hdlg, WX_ID(id));
                sprintf(s, "%imb",
                        (int)(((((uint64_t)hd_new_cyl * (uint64_t)hd_new_hpc) * (uint64_t)hd_new_spt) * 512) / 1024) / 1024);
                wx_sendmessage(h, WX_WM_SETTEXT, 0, (LONG_PARAM)s);
...
static int hd_file(void *hdlg, int drive) {
        if (!getfile(hdlg, "Hard disc image (*.img;*.vhd)|*.img;*.vhd|All files (*.*)|*.*", "")) {
                ...
                if (mvhd_file_is_vhd(f)) {
                        ...
                        if (vhd == NULL) {
                                ...
                                const char ts_warning[] =
                                        ...
                                        "This could also happen if the VHD files were moved/copied, or the differencing VHD was "
                ...
                if (wx_dialogbox(hdlg, "HdSizeDlg", hdsize_dlgproc) == 1) {
                        ...
                        h = wx_getdlgitem(hdlg, WX_ID(id));
                        sprintf(s, "%imb",
                                (int)(((((uint64_t)hd_new_cyl * (uint64_t)hd_new_hpc) * (uint64_t)hd_new_spt) * 512) / 1024) /
                                        1024);
                        wx_sendmessage(h, WX_WM_SETTEXT, 0, (LONG_PARAM)s);
...
int config_open(void *hwnd) { return wx_dialogbox(hwnd, "ConfigureDlg", config_dialog_proc); }
