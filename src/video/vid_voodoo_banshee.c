...
static void banshee_render_16bpp_tiled(svga_t *svga) {
        ...
        if (banshee->vidProcCfg & VIDPROCCFG_HALF_MODE)
                addr = banshee->desktop_addr + ((banshee->desktop_y >> 1) & 31) * 128 +
                       ((banshee->desktop_y >> 6) * banshee->desktop_stride_tiled);
        else
                addr = banshee->desktop_addr + (banshee->desktop_y & 31) * 128 +
...
static void banshee_recalctimings(svga_t *svga) {
        ...
        if (banshee->vgaInit0 & VGAINIT0_EXTENDED_SHIFT_OUT) {
                ...
                case PIX_FORMAT_RGB565:
                        svga->render = (banshee->vidProcCfg & VIDPROCCFG_DESKTOP_TILE) ? banshee_render_16bpp_tiled
                ...
                //                pclog("Extended shift out %i rowoffset=%i %02x\n", VIDPROCCFG_DESKTOP_PIX_FORMAT,
                //                svga->rowoffset, svga->crtc[1]);
                ...
                if (svga->overlay.ena) {
                        /*                        pclog("Overlay enabled : start=%i,%i end=%i,%i size=%i,%i pitch=%x\n",
                                                        ...
                                                        svga->overlay.pitch);*/
                        if (!voodoo->overlay.start_x && !voodoo->overlay.start_y && svga->hdisp == voodoo->overlay.size_x &&
                            svga->dispend == voodoo->overlay.size_y) {
...
static void banshee_ext_outl(uint16_t addr, uint32_t val, void *p) {
        ...
        case Video_vidSerialParallelPort:
                ...
                //                (VIDSERIAL_DDC_DCK_W | VIDSERIAL_DDC_DDA_W), CS,cs,cpu_state.pc);
...
static uint32_t banshee_status(banshee_t *banshee) {
        ...
        int busy = (written - voodoo->cmd_read) || (voodoo->cmdfifo_depth_rd != voodoo->cmdfifo_depth_wr) ||
                   voodoo->render_voodoo_busy[0] || voodoo->render_voodoo_busy[1] || voodoo->render_voodoo_busy[2] ||
                   voodoo->render_voodoo_busy[3] || voodoo->voodoo_busy;
        uint32_t ret;
        ...
        //        voodoo->cmd_written, voodoo->cmd_written_fifo, voodoo->cmd_read, voodoo->cmdfifo_depth_rd,
        //        voodoo->cmdfifo_depth_wr, CS,cs,cpu_state.pc, ret);
...
static uint32_t banshee_ext_inl(uint16_t addr, void *p) {
        ...
        case Video_vidSerialParallelPort:
                ...
                //                (VIDSERIAL_DDC_DCK_R | VIDSERIAL_DDC_DDA_R), CS,cs,cpu_state.pc);
                break;
...
static uint32_t banshee_reg_readl(uint32_t addr, void *p) {
        ...
        //        CS,cs,cpu_state.pc); if (cpu_state.pc == 0x1000e437)
        //                output = 3;
        return ret;
}
...
static void banshee_write_linear_l(uint32_t addr, uint32_t val, void *p) {
        ...
        *(uint32_t *)&svga->vram[addr & svga->vram_mask] = val;
        if (voodoo->cmdfifo_enabled && addr >= voodoo->cmdfifo_base && addr < voodoo->cmdfifo_end) {
                ...
                //                val, voodoo->cmdfifo_amin, voodoo->cmdfifo_amax, voodoo->cmdfifo_holecount,
                //                voodoo->cmdfifo_depth_wr, voodoo->cmdfifo_rp);
                if (addr == voodoo->cmdfifo_base && !voodoo->cmdfifo_holecount) {
                        ...
                        //                        if ((addr <= voodoo->cmdfifo_amin && voodoo->cmdfifo_amin != -4) || addr >=
                        //                        voodoo->cmdfifo_amax)
                        //                                fatal("CMDFIFO holecount write outside of amin/amax - amin=%08x
                        //                                amax=%08x holecount=%i\n", voodoo->cmdfifo_amin, voodoo->cmdfifo_amax,
        ...
        if (banshee->vidProcCfg & VIDPROCCFG_CURSOR_MODE) {
                ...
                for (x = 0; x < 64; x += 8) {
                        if (x_off > (32 - 8)) {
                                ...
                                for (xx = 0; xx < 8; xx++) {
                                        if (plane0[x >> 3] & (1 << 7))
                                                ((uint32_t *)buffer32->line[displine])[x_off + xx] =
                                                        (plane1[x >> 3] & (1 << 7)) ? col1 : col0;
...
#define DECODE_RGB565_TILED(buf)                                                                                                 \
        do {                                                                                                                     \
                ...
                for (c = 0; c < voodoo->overlay.overlay_bytes; c += 2) {                                                         \
                        uint16_t data =                                                                                          \
                                *(uint16_t *)&svga->vram[(base_addr + (c & 127) + (c >> 7) * 128 * 32) & svga->vram_mask];       \
                        int r = data & 0x1f;                                                                                     \
                        int g = (data >> 5) & 0x3f;                                                                              \
                        int b = data >> 11;                                                                                      \
                                                                                                                                 \
...
static void banshee_overlay_draw(svga_t *svga, int displine) {
        ...
        uint32_t src_addr2 =
                svga->overlay_latch.addr + ((banshee->vidProcCfg & VIDPROCCFG_OVERLAY_TILE)
        ...
        if (svga->render == svga_render_null && !svga->changedvram[src_addr >> 12] && !svga->changedvram[src_addr2 >> 12] &&
            !svga->fullchange && ((voodoo->overlay.src_y >> 20) < 2048 && !voodoo->dirty_line[voodoo->overlay.src_y >> 20]) &&
        ...
        //        pclog("displine=%i addr=%08x %08x  %08x  %08x\n", displine, svga->overlay_latch.addr, src_addr,
        //        voodoo->overlay.vidOverlayDvdy, *(uint32_t *)src); if (src_addr >= 0x800000)
        ...
        if (banshee->voodoo->scrfilter && banshee->voodoo->scrfilterEnabled)
                skip_filtering = ((banshee->vidProcCfg & VIDPROCCFG_FILTER_MODE_MASK) != VIDPROCCFG_FILTER_MODE_BILINEAR &&
                                  ...
                                  !(banshee->vidProcCfg & VIDPROCCFG_FILTER_MODE_DITHER_4X4) &&
        ...
        if (skip_filtering) {
                ...
                case VIDPROCCFG_FILTER_MODE_BILINEAR:
                        ...
                        if (banshee->vidProcCfg & VIDPROCCFG_H_SCALE_ENABLE) {
                                for (x = 0; x < svga->overlay_latch.xsize; x++) {
                                        ...
                                        unsigned int coeffs[4] = {((0x10000 - x_coeff) * (0x10000 - y_coeff)) >> 16,
                                        ...
                                        int r = (((samp0 >> 16) & 0xff) * coeffs[0] + ((samp1 >> 16) & 0xff) * coeffs[1] +
                                                 ((samp2 >> 16) & 0xff) * coeffs[2] + ((samp3 >> 16) & 0xff) * coeffs[3]) >>
                                                16;
                                        int g = (((samp0 >> 8) & 0xff) * coeffs[0] + ((samp1 >> 8) & 0xff) * coeffs[1] +
                                                 ((samp2 >> 8) & 0xff) * coeffs[2] + ((samp3 >> 8) & 0xff) * coeffs[3]) >>
                                                16;
                                        int b = ((samp0 & 0xff) * coeffs[0] + (samp1 & 0xff) * coeffs[1] +
                                                 (samp2 & 0xff) * coeffs[2] + (samp3 & 0xff) * coeffs[3]) >>
                                                16;
                                        p[x] = (r << 16) | (g << 8) | b;
                                ...
                                for (x = 0; x < svga->overlay_latch.xsize; x++) {
                                        ...
                                        int r = (((samp0 >> 16) & 0xff) * (0x10000 - y_coeff) +
                                                 ((samp1 >> 16) & 0xff) * y_coeff) >>
                                                16;
                                        int g = (((samp0 >> 8) & 0xff) * (0x10000 - y_coeff) + ((samp1 >> 8) & 0xff) * y_coeff) >>
                                                16;
                ...
                case VIDPROCCFG_FILTER_MODE_DITHER_2X2:
                        if (banshee->voodoo->scrfilter && banshee->voodoo->scrfilterEnabled) {
                                ...
                                {
                                        for (x = 0; x < svga->overlay_latch.xsize; x++) {
                                                p[x] = (fil[(src_x >> 20) * 3 + 2] << 16) | (fil[(src_x >> 20) * 3 + 1] << 8) |
                                                       fil[(src_x >> 20) * 3];
                                                src_x += voodoo->overlay.vidOverlayDudx;
                                        }
                                } else {
...
static void banshee_pci_write(int func, int addr, uint8_t val, void *p) {
        ...
        case PCI_REG_COMMAND:
                if (val & PCI_COMMAND_IO) {
                        ...
                        if (banshee->ioBaseAddr)
                                io_removehandler(banshee->ioBaseAddr, 0x0100, banshee_ext_in, NULL, banshee_ext_inl,
                                                 banshee_ext_out, NULL, banshee_ext_outl, banshee);

                        io_sethandler(0x03c0, 0x0020, banshee_in, NULL, NULL, banshee_out, NULL, NULL, banshee);
                        if (banshee->ioBaseAddr)
                                io_sethandler(banshee->ioBaseAddr, 0x0100, banshee_ext_in, NULL, banshee_ext_inl, banshee_ext_out,
                                              NULL, banshee_ext_outl, banshee);
                } else {
                        io_removehandler(0x03c0, 0x0020, banshee_in, NULL, NULL, banshee_out, NULL, NULL, banshee);
                        io_removehandler(banshee->ioBaseAddr, 0x0100, banshee_ext_in, NULL, banshee_ext_inl, banshee_ext_out,
                                         NULL, banshee_ext_outl, banshee);
                }
        ...
        case 0x19:
                ...
                if ((banshee->pci_regs[PCI_REG_COMMAND] & PCI_COMMAND_IO) && banshee->ioBaseAddr)
                        io_sethandler(banshee->ioBaseAddr, 0x0100, banshee_ext_in, NULL, banshee_ext_inl, banshee_ext_out, NULL,
                                      banshee_ext_outl, banshee);
...
static device_config_t banshee_sgram_config[] = {
        {.name = "memory",
         ...
         .selection = {{.description = "8 MB", .value = 8}, {.description = "16 MB", .value = 16}, {.description = ""}},
         .default_int = 16},
        {.name = "bilinear", .description = "Bilinear filtering", .type = CONFIG_BINARY, .default_int = 1},
        {.name = "dithersub", .description = "Dither subtraction", .type = CONFIG_BINARY, .default_int = 1},
        {.name = "dacfilter", .description = "Screen Filter", .type = CONFIG_BINARY, .default_int = 0},
        {.name = "render_threads",
         .description = "Render threads",
         .type = CONFIG_SELECTION,
         .selection = {{.description = "1", .value = 1},
                       {.description = "2", .value = 2},
                       {.description = "4", .value = 4},
                       {.description = ""}},
         .default_int = 2},
#ifndef NO_CODEGEN
        {.name = "recompiler", .description = "Recompiler", .type = CONFIG_BINARY, .default_int = 1},
#endif
        {.type = -1}};
...
static void *banshee_init_common(char *fn, int has_sgram, int type, int voodoo_type) {
        ...
        svga_init(&banshee->svga, banshee, mem_size << 20, banshee_recalctimings, banshee_in, banshee_out, banshee_hwcursor_draw,
                  banshee_overlay_draw);
        banshee->svga.vsync_callback = banshee_vsync_callback;

        mem_mapping_add(&banshee->linear_mapping, 0, 0, banshee_read_linear, banshee_read_linear_w, banshee_read_linear_l,
                        banshee_write_linear, banshee_write_linear_w, banshee_write_linear_l, NULL, MEM_MAPPING_EXTERNAL,
                        &banshee->svga);
        mem_mapping_add(&banshee->reg_mapping_low, 0, 0, banshee_reg_read, banshee_reg_readw, banshee_reg_readl,
                        banshee_reg_write, banshee_reg_writew, banshee_reg_writel, NULL, MEM_MAPPING_EXTERNAL, banshee);
        mem_mapping_add(&banshee->reg_mapping_high, 0, 0, banshee_reg_read, banshee_reg_readw, banshee_reg_readl,
                        banshee_reg_write, banshee_reg_writew, banshee_reg_writel, NULL, MEM_MAPPING_EXTERNAL, banshee);
...
static void *banshee_init() { return banshee_init_common("pci_sg.rom", 1, TYPE_BANSHEE, VOODOO_BANSHEE); }
static void *creative_banshee_init() { return banshee_init_common("blasterpci.rom", 0, TYPE_BANSHEE, VOODOO_BANSHEE); }
static void *v3_2000_init() { return banshee_init_common("voodoo3_2000/2k11sd.rom", 0, TYPE_V3_2000, VOODOO_3); }
static void *v3_3000_init() { return banshee_init_common("voodoo3_3000/3k12sd.rom", 0, TYPE_V3_3000, VOODOO_3); }

static int banshee_available() { return rom_present("pci_sg.rom"); }
static int creative_banshee_available() { return rom_present("blasterpci.rom"); }
static int v3_2000_available() { return rom_present("voodoo3_2000/2k11sd.rom"); }
static int v3_3000_available() { return rom_present("voodoo3_3000/3k12sd.rom"); }
...
static void banshee_add_status_info(char *s, int max_len, void *p) {
        ...
        pixel_count_total = (pixel_count_current[0] + pixel_count_current[1] + pixel_count_current[2] + pixel_count_current[3]) -
                            (voodoo->pixel_count_old[0] + voodoo->pixel_count_old[1] + voodoo->pixel_count_old[2] +
                             voodoo->pixel_count_old[3]);
        texel_count_total = (texel_count_current[0] + texel_count_current[1] + texel_count_current[2] + texel_count_current[3]) -
                            (voodoo->texel_count_old[0] + voodoo->texel_count_old[1] + voodoo->texel_count_old[2] +
                             voodoo->texel_count_old[3]);
        sprintf(temps,
                "%f Mpixels/sec (%f)\n%f Mtexels/sec (%f)\n%f ktris/sec\n%f%% CPU (%f%% real)\n%d frames/sec (%i)\n%f%% CPU "
                "(%f%% real)\n" /*%d reads/sec\n%d write/sec\n%d tex/sec\n*/,
                ...
                (double)voodoo->tri_count / 1000.0, ((double)voodoo->time * 100.0) / timer_freq,
                ((double)voodoo->time * 100.0) / status_diff, voodoo->frame_count, voodoo_recomp,
                ((double)voodoo->render_time[0] * 100.0) / timer_freq, ((double)voodoo->render_time[0] * 100.0) / status_diff);
        if (voodoo->render_threads >= 2) {
                char temps2[512];
                sprintf(temps2, "%f%% CPU (%f%% real)\n", ((double)voodoo->render_time[1] * 100.0) / timer_freq,
                        ((double)voodoo->render_time[1] * 100.0) / status_diff);
        ...
        if (voodoo->render_threads == 4) {
                ...
                sprintf(temps2, "%f%% CPU (%f%% real)\n%f%% CPU (%f%% real)\n",
                        ...
                        ((double)voodoo->render_time[2] * 100.0) / status_diff,
                        ((double)voodoo->render_time[3] * 100.0) / timer_freq,
                        ((double)voodoo->render_time[3] * 100.0) / status_diff);
                strncat(temps, temps2, sizeof(temps) - 1);
        }
...
device_t voodoo_banshee_device = {"Voodoo Banshee PCI (reference)",
                                  DEVICE_PCI,
                                  banshee_init,
                                  banshee_close,
                                  banshee_available,
...
device_t creative_voodoo_banshee_device = {"Creative Labs 3D Blaster Banshee PCI",
                                           DEVICE_PCI,
...
device_t voodoo_3_2000_device = {"Voodoo 3 2000 PCI",  DEVICE_PCI,
                                 v3_2000_init,         banshee_close,
                                 v3_2000_available,    banshee_speed_changed,
                                 banshee_force_redraw, banshee_add_status_info,
                                 banshee_sdram_config};

device_t voodoo_3_3000_device = {"Voodoo 3 3000 PCI",  DEVICE_PCI,
                                 v3_3000_init,         banshee_close,
                                 v3_3000_available,    banshee_speed_changed,
                                 banshee_force_redraw, banshee_add_status_info,
                                 banshee_sdram_config};
