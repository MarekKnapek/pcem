...
enum host_cpu_feature { HOST_CPU_MMX = 1, HOST_CPU_SSE = 2, HOST_CPU_SSE2 = 4, HOST_CPU_SSE3 = 8 };
...
static cpu_x86_regs_t get_cpuid_regs(unsigned int index) {
	  ...
	  mov [retval.edx], edx
        ...
        asm("movl %1, %%eax; cpuid; movl %%ecx, %0;" : "=m"(retval.ecx) : "r"(index) : "eax", "ebx", "ecx", "edx");
...
static int host_cpu_features(void) { return 0; }
#endif
...
SIDFP::State SIDFP::read_state() {
        ...
        for (i = 0, j = 0; i < 3; i++, j += 7) {
                ...
                state.sid_register[j + 4] = (wave.waveform << 4) | (wave.test ? 0x08 : 0) | (wave.ring_mod ? 0x04 : 0) |
                                            (wave.sync ? 0x02 : 0) | (envelope.gate ? 0x01 : 0);
        ...
        state.sid_register[j++] = (filter.voice3off ? 0x80 : 0) | (filter.hp_bp_lp << 4) | filter.vol;
...
// Enable filter.
// ----------------------------------------------------------------------------
void SIDFP::enable_filter(bool enable) { filter.enable_filter(enable); }

// ----------------------------------------------------------------------------
// Enable external filter.
// ----------------------------------------------------------------------------
void SIDFP::enable_external_filter(bool enable) { extfilt.enable_filter(enable); }
...
// not overfilled.
// ----------------------------------------------------------------------------
bool SIDFP::set_sampling_parameters(float clock_freq, sampling_method method, float sample_freq, float pass_freq) {
        ...
        for (int i = 0; i < fir_RES; i++) {
                ...
                for (int j = 0; j < fir_N; j++) {
                        ...
                        double sincwt = fabs(wt) >= 1e-8 ? sin(wt) / wt : 1;
...
// ----------------------------------------------------------------------------
void SIDFP::adjust_sampling_frequency(float sample_freq) { cycles_per_sample = clock_frequency / sample_freq; }
...
// ----------------------------------------------------------------------------
RESID_INLINE
int SIDFP::clock_interpolate(cycle_count &delta_t, short *buf, int n, int interleave) {
        int s = 0;
        int i;
...
// ----------------------------------------------------------------------------
RESID_INLINE
int SIDFP::clock_resample_interpolate(cycle_count &delta_t, short *buf, int n, int interleave) {
        int s = 0;
