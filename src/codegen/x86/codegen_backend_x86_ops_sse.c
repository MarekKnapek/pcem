...
void host_x86_MOVQ_BASE_OFFSET_XREG(codeblock_t *block, int base_reg, int offset, int src_reg) {
        if (offset >= -128 && offset < 127) {
                if (base_reg == REG_ESP) {
                        ...
                        codegen_alloc_bytes(block, 5);
                        codegen_addbyte4(block, 0x66, 0x0f, 0xd6,
                                         0x40 | base_reg | (src_reg << 3)); /*MOVQ [base_reg + offset], XMMx*/
                        codegen_addbyte(block, offset);
                }
        } else
...
void host_x86_MOVQ_XREG_BASE_OFFSET(codeblock_t *block, int dst_reg, int base_reg, int offset) {
        if (offset >= -128 && offset < 127) {
                if (base_reg == REG_ESP) {
                        ...
                        codegen_alloc_bytes(block, 5);
                        codegen_addbyte4(block, 0xf3, 0x0f, 0x7e,
                                         0x40 | base_reg | (dst_reg << 3)); /*MOVQ XMMx, [base_reg + offset]*/
                        codegen_addbyte(block, offset);
                }
        } else
...
void host_x86_PACKSSWB_XREG_XREG(codeblock_t *block, int dst_reg, int src_reg) {
        ...
        codegen_addbyte4(block, 0x66, 0x0f, 0x70,
                         0xc0 | dst_reg | (dst_reg << 3)); /*PSHUFD dst_reg, dst_reg, 0x88 (move bits 64-95 to 32-63)*/
...
void host_x86_PSHUFD_XREG_XREG_IMM(codeblock_t *block, int dst_reg, int src_reg, uint8_t shuffle) {
        codegen_alloc_bytes(block, 5);
        codegen_addbyte4(block, 0x66, 0x0f, 0x70,
                         0xc0 | src_reg | (dst_reg << 3)); /*PSHUFD dst_reg, dst_reg, 0xee (move top 64-bits to low 64-bits)*/
